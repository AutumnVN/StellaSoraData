...
local function __printTable(tb, sb, name, aPreText, bLast)
	...
	if aPreText == nil then
		...
		if bLast then
			...
			subPreText = preText .. "\226\148\130  "
		end
		if type(tb) == "table" then
			if bLast then
				sb:AppendLine(string.format("%s\226\148\148\226\148\128[%s]", preText, name))
			else
				sb:AppendLine(string.format("%s\226\148\156\226\148\128[%s]", preText, name))
			end
		elseif bLast then
			sb:AppendLine(string.format("%s\226\148\148\226\148\128%s= %s", preText, name, tostring(tb)))
		else
			sb:AppendLine(string.format("%s\226\148\156\226\148\128%s= %s", preText, name, tostring(tb)))
		end
	end
	if type(tb) == "table" then
		local counter = 0
