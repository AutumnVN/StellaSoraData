		...
		if mapCfgData ~= nil then
			if mapCfgData.Type == GameEnum.itemType.Char then
				...
				if mapCharCfg ~= nil then
					ret = string.format("%s\194\183%s", ConfigTable.GetUIText("T_Element_Attr_" .. mapCharCfg.EET), mapCharCfg.Name)
				end
			elseif mapCfgData.Type == GameEnum.itemType.Disc then
				...
				if mapDiscCfg ~= nil then
					ret = string.format("%s\194\183%s", ConfigTable.GetUIText("T_Element_Attr_" .. mapDiscCfg.EET), mapCfgData.Title)
				end
			end
		end
		return ret
