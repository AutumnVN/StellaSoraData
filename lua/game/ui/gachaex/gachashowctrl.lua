...
function GachaShowCtrl:ShowItem(mapItem)
	...
	if mapItem.rewardItem ~= nil and #mapItem.rewardItem > 0 then
		...
		if mapRewardItemCfgData == nil then
			printError("item ID missing\239\188\154" .. mapItem.rewardItem[1].Tid)
	...
	if mapItem.exItem ~= nil and 0 < #mapItem.exItem then
		...
		if mapRewardItemCfgData == nil then
			printError("item ID missing\239\188\154" .. mapItem.exItem[1].Tid)
			...
			NovaAPI.SetTMPText(self._mapNode.TMP_ItemCount2, string.format("\195\151%d", tostring(nRewardItemCount)))
		end
	else
		self._mapNode.rtItemTips2:SetActive(false)
	end
