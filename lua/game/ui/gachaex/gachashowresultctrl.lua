	...
	local AllCardShowCallback = function()
		...
		for i = 1, 3 do
			if tbItem[i] ~= nil then
				...
				if mapItemCfgData == nil then
					...
					NovaAPI.SetTMPText(self._mapNode.TMPItemCount[i], string.format("\195\151%d", tbItem[i].nQty))
					bRewardActive = true
				end
			else
				self._mapNode.rtItem[i].gameObject:SetActive(false)
		...
		if tbGiveItems[1] ~= nil then
			...
			if mapItemCfgData == nil then
				...
				NovaAPI.SetTMPText(self._mapNode.TMPItemCount[3], string.format("\195\151%d", tbGiveItems[1].Qty))
				bRewardActive = true
			end
		else
			self._mapNode.rtItem[3].gameObject:SetActive(false)
