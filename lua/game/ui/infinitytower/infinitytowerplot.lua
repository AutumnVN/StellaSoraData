...
function InfinityTowerPlot:InitPlotList(goGrid, gridIndex)
	...
	if isUnLock then
		...
		if isGet then
			...
			NovaAPI.SetTMPText(plot_item_RewardCount, "\195\151" .. plotData.RewardItemQty)
...
function InfinityTowerPlot:ShowGoPlotPanel(plotData)
	...
	if isGet then
		...
		NovaAPI.SetTMPText(self._mapNode.txtGoPlotRewardCount, "\195\151" .. plotData.RewardItemQty)
